...
3. **Layout is permanent:** `${OPS_ROOT:-~/ops}/stayturgid` (public product),
   `${OPS_ROOT:-~/ops}/site-djbclark` (private site). Never nest a private repo inside a
...
| Virtualenvs embed absolute interpreter paths                                                                          | `.venv-test` broke when the repo moved to `${OPS_ROOT:-~/ops}` (rebuilt); if a venv errors with "bad interpreter", `rm -rf` it and rerun `just test-venv` — never edit shebangs by hand                             |
...
| --- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------- | -------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------- |
| B1  | Create `inventory/` here: copy live `hosts.yml` + `group_vars/stayturgid.yml` from stayturgid; add site `ansible.cfg` (inventory here, `collections_path` → `${OPS_ROOT:-~/ops}/stayturgid/.ansible/collections` + `ansible_collections/`); add thin `justfile` wrapper exporting `ANSIBLE_CONFIG`                            | 45         | Codex (high) or Sonnet 5                                                                                       | Verify: `ansible-inventory --list` from this repo matches the one from stayturgid byte-for-byte (normalize with `jq -S`)     |
