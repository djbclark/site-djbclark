...
>
> **Agent rules (always read on handoff):** root [`AGENTS.md`](https://github.com/djbclark/stayturgid/blob/master/AGENTS.md),
> [coding-rules.md](https://github.com/djbclark/stayturgid/blob/master/docs/coding-rules.md), and [`.cursor/rules/`](https://github.com/djbclark/stayturgid/blob/master/.cursor/rules/).
...
>
> **2026-07-18 (later):** **Branch consolidation — master is the only branch.**
> Merged: PR #2 (topology/ADR 005/site-contract spec/CA scrub),
> `feature/platform-arch` (identity SSoT: `just validate-identity`, warn-only,
> 193 known violations), `feature/logging` (Ansible-managed vector+openobserve
> via `observability.yml`; on-device JSONL + `state.json` dual-write), and
> `just-standardization` (recovered disk WIP). All remote/local feature
> branches and old worktrees (`stayturgid_work`, `stayturgid.d/*`,
...
**Location:** [`.cursor/rules/`](https://github.com/djbclark/stayturgid/blob/master/.cursor/rules/) (repo root; not under `docs/`).
...
| File                                                                                                                                                    | Topic                                                                                                               |
| ------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------- |
| [`.cursor/rules/fleet-health-self-heal.mdc`](https://github.com/djbclark/stayturgid/blob/master/.cursor/rules/fleet-health-self-heal.mdc)               | Health fixes must also update self-heal paths (Termux / AutoJs6 / Mac monitors) — session-only heals are incomplete |
| [`.cursor/rules/screen-control-hold.mdc`](https://github.com/djbclark/stayturgid/blob/master/.cursor/rules/screen-control-hold.mdc)                     | Keep `ScreenControlSession` held across multi-step UI work; do not open/close per tap                               |
| [`.cursor/rules/deploy-self-heal-catastrophic.mdc`](https://github.com/djbclark/stayturgid/blob/master/.cursor/rules/deploy-self-heal-catastrophic.mdc) | Every capability must be in deploy, self-heal, **and** catastrophic recovery — no gaps                              |
...
Then read [coding-rules.md](https://github.com/djbclark/stayturgid/blob/master/docs/coding-rules.md), [docs/options.md](https://github.com/djbclark/stayturgid/blob/master/docs/options.md), and the
[ordered outstanding-fix plan](https://github.com/djbclark/stayturgid/blob/master/docs/operations/plans/outstanding-fix-priorities-2026-07-13.md)
...
[the execution plan](https://github.com/djbclark/stayturgid/blob/master/docs/operations/plans/outstanding-fix-priorities-2026-07-13.md) for gates and
rollback rules.
...
- **hd8 AutoJs6 project recovery (2026-07-13):** AutoJs6 would not recognize the repo tree as a project until the app-created project root was used. The working pair that brought hd8 back is saved under [docs/research/autojs6-hd8-project/](https://github.com/djbclark/stayturgid/blob/master/docs/research/autojs6-hd8-project/); keep that as a reference for project-recognition behavior.
...
- shell-gpt / local LLM (incubator): [docs/research/experiments/on-device-llm.md](https://github.com/djbclark/stayturgid/blob/master/docs/research/experiments/on-device-llm.md) (OPTIONS **54** only if asked).
- Parked side projects: [docs/research/experiments/](https://github.com/djbclark/stayturgid/blob/master/docs/research/experiments) — Inferno/Styx **do not implement**.
...
live status in [options.md](https://github.com/djbclark/stayturgid/blob/master/docs/options.md). Start with the first incomplete priority.
The dashboard-framework research prompt is at
[docs/research/prompts/dashboard-framework-research.md](https://github.com/djbclark/stayturgid/blob/master/docs/research/prompts/dashboard-framework-research.md).
...
| ⚠ Power            | the USB _data_ cable does NOT reliably charge it — keep it on a real charger or remote access dies with the battery                            |

Prefer the **S24 over USB** for interactive work when plugged in; use **7a over Tailscale** otherwise. Mac scripts resolve targets via [control/lib/stayturgid_device.py](https://github.com/djbclark/stayturgid/blob/master/control/lib/stayturgid_device.py) or `./control/lib/resolve_adb.py` (USB serial when present, else Tailscale/LAN).
...
Bench: [docs/research/handsets-vs-u2-bench.md](https://github.com/djbclark/stayturgid/blob/master/docs/research/handsets-vs-u2-bench.md).
...
> (54) — see [docs/options.md](https://github.com/djbclark/stayturgid/blob/master/docs/options.md).
...
| **E — On-device LLM**       | shell-gpt escalation after deterministic heal; see [docs/research/experiments/on-device-llm.md](https://github.com/djbclark/stayturgid/blob/master/docs/research/experiments/on-device-llm.md) | Rare adaptive repair; never hot-path |

<!-- markdownlint-enable MD060 -->

**Parked (not equal-weight):** Inferno/Styx and similar experiments live under
[docs/research/experiments/](https://github.com/djbclark/stayturgid/blob/master/docs/research/experiments) — agents must not work on them unless the
...
catastrophic-heal reasons ([incubator analysis](https://github.com/djbclark/stayturgid/blob/master/docs/research/experiments/inferno-styx/analysis.md)).
...
[std_modules_audit.md](https://github.com/djbclark/stayturgid/blob/master/docs/ansible/collections/std_modules_audit.md).
...
configures, or health-checks them. Re-enable: [docs/architecture/components/fdroid.md](https://github.com/djbclark/stayturgid/blob/master/docs/architecture/components/fdroid.md),
[docs/architecture/components/play.md](https://github.com/djbclark/stayturgid/blob/master/docs/architecture/components/play.md).
