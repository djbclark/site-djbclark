...
- **Secrets**: Use `secretspec.toml` to project a persistent `WEBUI_SECRET_KEY` into the plist via ambient env mode 0600 (prevents session invalidation on restart). Do _not_ invent a LiteLLM master key.
